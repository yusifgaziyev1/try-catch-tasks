...
try {
    let getFindEndFirstName = users.find((firstName) => firstName.firstName.endsWith("q"));
    if (getFindEndFirstName != undefined) {
        console.log(getFindEndFirstName);
    } else {
        throw new Error("Users listde sonu q herfi ile biten insan adı yoxdur");
    }
...
try {
    let getFindUnderAgeOneHundredOne = users.filter((bigAge) => bigAge.age > 100);
    if (getFindUnderAgeOneHundredOne.length > 0) {
        console.log(getFindUnderAgeOneHundredOne);
    } else {
        throw new Error("Yaşı 100'den yuxarı olan istifadeci mövcud deyil.");    
    }
    
} catch (error) {
    console.error(error.message);
} finally {
    let allUsersAges = users.map((ages) => ages.age);
    console.log(allUsersAges)
}
...
function getFindId(id) {
    let result = users.find((findId) => findId.id == id);    
    try {
        if (result == undefined) {
            throw new Error(`${id} id nömreli istifadəçi tapılmadı.`);
        } else {
            console.log(result)
        }
    } catch (error) {
...
function getIdFindAddres(id) {
    let result = users.find((getAddress) => getAddress.id == id && getAddress.address);
    try {
        if (result) {
            console.log(result);
        } else {
            throw new Error(`${id} id nömrəli istifadəçi tapılmadı`);
        }
    } catch (error) {
        console.error(error.message);
    }
...
function getFindWeightAndHeight(id) {
    let result = users.find((getWeightHeight) => getWeightHeight.id == id);
    try {
        if (result) {
            console.log(result.height);
            console.log(result.weight);
        } else {
            throw new Error(`${id} id nömreli istifadəçinin boy və ya kilo məlumatı eksikdi.`);
...
getFindWeightAndHeight(55)

// Qeyd Array yenilib burdaki herseyi kopyalayib burda array uzerinden tasklari edin recordlara baxin
// yazdiginiz kodlari tekrar tekrar yazin mahiyetini basa dusun ugurlar :)
